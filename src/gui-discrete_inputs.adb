package body GUI.Discrete_Inputs is
   ...
   begin
      ...
      for I in T loop
         Element.Add_Option (Value => UXStrings.From_Latin_1 (I'Image), Text => UXStrings.From_Latin_1 (I'Image));
      end loop;
   end Create;

   function Get (Input : Discrete_Input) return T is
   begin
      return T'Value (Input.Value.To_Latin_1);
   end Get;

   procedure Set (Input : in out Discrete_Input; Value : T) is
   begin
      for I in 1 .. Input.Length loop
         Input.Selected (I, Input.Value (I) = UXStrings.From_Latin_1 (Value'Image));
      end loop;
   end Set;
